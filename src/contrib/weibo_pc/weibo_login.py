...
Rewrite on Mar 05, 2014

@author: yoyzhou
@rewrite: winkidney@gmail.com
'''
...
__site__= "http://yoyzhou.github.com,http://github.com/winkidney/"
...
def login(username, pwd, cookie_file):
    """"
        Login with use name, password or cookies.
        (1) If cookie file exists then try to load cookies,
            return a opener with the weibo logined cookies.
        (2) If no cookies found then do login and return a opener with logined cookie.
            now you can descide whether you will install the opener:)
    """
...
def do_login(username,pwd,cookie_file):
    ...
    try:
        ...
        data = opener2.open(login_url).read()
...
def main():    
    ...
    cw_opener = login(username, pwd, cookie_file)
    if cw_opener :
